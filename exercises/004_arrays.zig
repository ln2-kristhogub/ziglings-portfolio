...
pub fn main() void {
    ...
    // How do we fix it?
    // ➜ FIX: Change 'const' to 'var' because we modify the array later.
    ...
    // ➜ FIX: Arrays are zero-indexed, so the 4th element is at index 3.
    const fourth = some_primes[3];

    // (Problem 3)
    // Use the len property to get the length of the array:
    // ➜ FIX: Use '.len' to find the total number of elements in the array.
...
//   var foo: [3]u32 = [3]u32{ 42, 108, 5423 };
//
// When Zig can infer the size of the array, you ca' for the
// size. You can also let Zig infer the type of the value so the
// declaration is much less verbose.
//
//   var foo = [_]u32{ 42, 108, 5423 };
...
pub fn main() void {
    ...
    // This "const" is going to cause a problem later - can you see what it is?
    // How do we fix it?
    const some_primes = [_]i32{ 3, 5, 7, 11, 13 };
